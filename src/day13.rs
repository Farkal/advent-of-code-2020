...
pub fn part1(input: &str) -> u32 {
    ...
    let boats: Vec<u32> = tmp_i.next().unwrap().split(',').filter(|x| x != &"x").map(|x| x.parse::<u32>().unwrap()).collect();
...
pub fn part2(input: &str) -> i64 {
    let mut tmp_i = input.lines();
    tmp_i.next().unwrap();
    let boats: Vec<(usize, i64)> = tmp_i.next().unwrap().split(',').enumerate().filter(|(_, x)| x != &"x").map(|(i, x)| (i, x.parse::<i64>().unwrap())).collect();
